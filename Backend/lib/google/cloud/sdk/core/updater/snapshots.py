...
class ComponentSnapshot(object):
  ...
  def FromURL(url):
    ...
    try:
      ...
      response = None
    if not response:
      raise URLFetchError('Unable to fetch %s' % url)
    ...
    return ComponentSnapshot.FromDictionary(data, url=url)
  ...
  @staticmethod
  def FromDictionary(json_dictionary, url=None):
