...
from google.cloud.sdk.core.updater import installers
from google.cloud.sdk.core.updater import local_state
from google.cloud.sdk.core.updater import schemas
from google.cloud.sdk.core.updater import snapshots
...
class UpdateManager(object):
  ...
  def _GetStateAndDiff(self):
    ...
    try:
      latest_snapshot = snapshots.ComponentSnapshot.FromURL(self.__url)
  ...
  def GetCurrentVersionsInformation(self):
    ...
    Returns:
      {str:str}, A mapping from component id to verison string.
    """
  ...
  def Update(self, update_seed=None, allow_no_backup=False):
    ...
    bad_commands = self._FindAllOldToolsOnPath()
    if bad_commands:
      log.warning("""\
...
""".format('\n'.join(bad_commands)))
  ...
  def _FindAllOldToolsOnPath(self, path=None):
  ...
  def _DoFreshInstall(self, e):
    ...
    installed_component_ids = install_state.InstalledComponents().keys()
    env = dict(os.environ)
    ...
    p = subprocess.Popen([sys.executable, '-S', installer_path],
