...
# This regular expression is used to extract the HEAD branch from a remote
# git repository by scraping 'git remote show origin'.
_HEAD_BRANCH_RE = re.compile(r'HEAD branch: (?P<branch>.*)')
...
class NoWorkspaceException(Error):
  def __init__(self, path):
    ...
    """
    message = ('Cannot find workspace in [{0}].'
               .format(path))
...
class CannotCreateWorkspaceException(Error):
  def __init__(self, path):
    ...
    """
    message = ('Cannot create workspace at [{0}].'
               .format(path))
...
def Create(root_directory):
  ...
  log.Print('Initialized gcloud directory in [{path}].'.format(
...
class Workspace(object):
  ...
  def CloneProjectRepository(self, project, alias=DEFAULT_REPOSITORY_ALIAS):
    ...
    """
    uri = _GetRepositoryURI(project, alias)
    if not uri:
      raise UnknownRepositoryAliasException()

    log.Print('Cloning [{uri}] into [{path}].'.format(
        uri=uri, path=alias))
  ...
  def CloneGitRepository(self, repository_url, repository_path):
    ...
    files.MakeDir(abs_repository_path)
    ...
    with files.ChDir(abs_repository_path) as _:
      ...
      if (repository_url.startswith('https://code.google.com') or
        ...
        subprocess.check_call(
            ['git', 'config', 'credential.helper', helper_name])
      ...
      try:
        ...
        # This directory was created in this command, so when things go wrong
        # we are safe deleting it.
        files.RmTree(abs_repository_path)
        raise CannotFetchRepositoryException('Could not fetch repository.')

      # we need to figure out what branch to checkout
      output = subprocess.check_output(['git', 'remote', 'show', 'origin'])
      output_match = _HEAD_BRANCH_RE.search(output)
      if output_match:
        head_branch = output_match.group('branch')
        if head_branch != '(unknown)':
          subprocess.check_call(['git', 'checkout', head_branch])
      else:
        log.warn('Unable to identify correct branch to checkout.')
