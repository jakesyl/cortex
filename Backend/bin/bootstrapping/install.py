...
Enter path to an rc file to update, or leave blank to use [{rc_path}]:  """
  ...
  if path_update:
    ...
    print ('Source [{path_rc_path}] in your profile to add the '
           'Google Cloud SDK command line tools to your $PATH.'.format(
               path_rc_path=path_rc_path))
  ...
  if bash_completion:
    ...
    print ('Source [{completion_rc_path}] in your profile to enable '
           'bash completion for gcloud.'.format(
               completion_rc_path=completion_rc_path))
...
def InstallComponents(component_ids):
  ...
  components = gcloud.gcloud(user_output_enabled=True,
